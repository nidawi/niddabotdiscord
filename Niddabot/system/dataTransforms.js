 ...
 * @param {DiscordMessageData} msg Discordjs message object.
 */
module.exports = async msg => {
  const delegate = msg.messageContent.getArgument('delegate')
  if (delegate) {
    // There is an active delegation request.
    const initiator = await msg.niddabot.cache.get('user', msg.author.id)
    const target = await msg.niddabot.cache.get('user', delegate)
    if (!initiator.canPerform(999)) throw new NiddabotError('you are not authorized to use the delegation feature.')
    if (!initiator.outranks(target.getPrivilege())) throw new NiddabotError('you are not authorized to issue a delegation towards that user.')
